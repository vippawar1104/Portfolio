        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex-shrink-0 md:hidden">
              <span className="text-zinc-100 font-bold text-lg px-2 font-mono">VP</span>
            </div>
              ...
              <div className="flex items-center justify-center space-x-4 lg:space-x-8">
                {navItems.map((item) => (
                  <Link
                    ...
                    className="relative text-zinc-400 hover:text-white px-3 py-2 rounded-md text-sm font-medium transition-all duration-300 hover:scale-105 hover:-translate-y-1 group overflow-hidden"
                  >
                    <span className="relative z-10">{item.name}</span>
                    <div className="absolute inset-0 transform -translate-x-full group-hover:translate-x-0 transition-transform duration-500 ease-out bg-white/10"></div>
                    <div className="absolute bottom-0 left-0 w-0 h-0.5 group-hover:w-full transition-all duration-500 ease-out bg-white"></div>
                  </Link>
                ))}
              </div>
            </div>
        ...
        <div className="px-6 py-4 space-y-1">
          {navItems.map((item, index) => (
            <Link
              ...
              href={item.href}
              onClick={(e) => handleNavClick(e, item.href)}
              className="text-zinc-400 hover:text-white block px-4 py-3 rounded-2xl text-base font-medium transition-all duration-300 active:bg-white/10 hover:bg-white/10"
              style={{
                animation: isOpen
